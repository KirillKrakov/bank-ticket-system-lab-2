...
public class ApplicationIntegrationTest {
    ...
    void listApplications_withPagination_shouldReturnPage() {
        // Создаем несколько заявок напрямую в БД для теста
        for (int i = 0; i < 5; i++) {
    ...
    @Test
    void streamApplications_shouldReturnWithCursor() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            app.setProductId(productId);
            app.setStatus(ApplicationStatus.SUBMITTED);
            app.setCreatedAt(java.time.Instant.now().plusSeconds(i)); // Разное время для сортировки
            applicationRepository.save(app);
        }

        ResponseEntity<List<ApplicationDto>> response = restTemplate.exchange(
                "/api/v1/applications/stream?limit=3",
                HttpMethod.GET,
                null,
        ...
        // Проверяем наличие курсора для следующей страницы
        String nextCursor = response.getHeaders().getFirst("X-Next-Cursor");
    ...
    void changeStatus_managerChangingOwnApplication_shouldReturnConflict() {
        // Создаем заявку менеджера
        Application app = new Application();
        app.setId(UUID.randomUUID());
        app.setApplicantId(managerId); // Менеджер является заявителем
        app.setProductId(productId);
        app.setStatus(ApplicationStatus.SUBMITTED);
        app.setCreatedAt(java.time.Instant.now());
        applicationRepository.save(app);
    ...
    void changeStatus_withoutPermissions_shouldReturnForbidden() {
        ...
        ResponseEntity<ApplicationDto> response = restTemplate.exchange(
                ...
                ApplicationDto.class,
                app.getId(),
                applicantId // Обычный клиент не может менять статус
        );
    ...
    void getApplicationHistory_withoutPermissions_shouldReturnForbidden() {
        ...
        ResponseEntity<List<ApplicationHistoryDto>> response = restTemplate.exchange(
                ...
                new ParameterizedTypeReference<List<ApplicationHistoryDto>>() {},
                app.getId(),
                applicantId // Чужой пользователь
        );
