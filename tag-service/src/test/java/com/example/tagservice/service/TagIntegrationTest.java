...
public class TagIntegrationTest {
    ...
    @Test
    void getTagByName_shouldReturnTagWithApplications() {
        // Given - create tag in database
        Tag tag = new Tag();
        ...
        // Note: ApplicationServiceClient fallback возвращает пустой список приложений
        ...
        assertEquals("test-tag", response.getBody().getName());
        assertNotNull(response.getBody().getApplications());
        // Fallback возвращает пустой список
        assertEquals(0, response.getBody().getApplications().size());
    }
    ...
    @Test
    void listTags_withPagination_shouldReturnPage() {
    ...
    @Test
    void tagLifecycle_createGetList() {
        // 1. Create multiple tags
        List<String> tagNames = Arrays.asList("feature", "bug", "enhancement");
        ...
        for (String tagName : tagNames) {
            ...
            ResponseEntity<TagDto> createResponse = restTemplate.postForEntity(
                    "/api/v1/tags",
                    entity,
                    TagDto.class
            );
        ...
        // 2. List tags
        ResponseEntity<TagDto[]> listResponse = restTemplate.getForEntity(
                "/api/v1/tags?page=0&size=10",
                TagDto[].class
        );
        ...
        // 3. Get each tag individually
        for (String tagName : tagNames) {
            ResponseEntity<TagDto> getResponse = restTemplate.getForEntity(
                    "/api/v1/tags/{name}",
                    TagDto.class,
                    tagName
            );
            ...
            assertEquals(tagName, getResponse.getBody().getName());

            // Applications should be empty (fallback)
            assertNotNull(getResponse.getBody().getApplications());
            assertEquals(0, getResponse.getBody().getApplications().size());
        }

        // 4. Use batch endpoint
        List<String> newTags = Arrays.asList("urgent", "documentation");
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<List<String>> batchEntity = new HttpEntity<>(newTags, headers);

        ResponseEntity<TagDto[]> batchResponse = restTemplate.postForEntity(
                "/api/v1/tags/batch",
                batchEntity,
                TagDto[].class
        );
        ...
        assertNotNull(batchResponse.getBody());
        assertEquals(2, batchResponse.getBody().length);
