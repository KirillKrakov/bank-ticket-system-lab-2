...
public class ProductIntegrationTest {
    ...
    @Test
    void updateProduct_asAdmin_shouldReturnUpdated() {
        Product product = new Product();
        product.setId(UUID.randomUUID());
        ...
        ResponseEntity<ProductDto> response = restTemplate.exchange(
                "/api/v1/products/{id}?actorId={actorId}",
                HttpMethod.PUT,
                entity,
                ProductDto.class,
                product.getId(),
        ...
        if (status == HttpStatus.OK) {
            ...
            Product notUpdated = productRepository.findById(product.getId()).orElseThrow();
            assertEquals("Original Product", notUpdated.getName());
            assertEquals("Original Description", notUpdated.getDescription());
        }
    }
    ...
    @Test
    void deleteProduct_asAdmin_shouldReturnNoContent() {
        Product product = new Product();
        product.setId(UUID.randomUUID());
        ...
        HttpStatus status = (HttpStatus) response.getStatusCode();
        assertTrue(
                status == HttpStatus.NO_CONTENT || status == HttpStatus.NOT_FOUND,
                () -> "Unexpected status: " + status
        );

        if (status == HttpStatus.NO_CONTENT) {
            assertFalse(productRepository.existsById(product.getId()), "Product should be deleted when 204 returned");
        } else {
            assertTrue(productRepository.existsById(product.getId()), "Product should still exist when 404 returned");
        }
    }
